...
export const initializeSocket = (server: HttpServer) => {
  ...
  chatNamespace.on(
    ...
    (
      ...
      socket.on(
        ...
        async ({
          ...
          try {
            ...
            if (!chatGroup.participants.includes(userId as unknown as ObjectId))
              throw new AppError(403, "Unauthorized access to chat group");
            ...
            await chatGroupService.updateLastMessage(
              chatGroupId,
              message._id!.toString()
            );
 
            // retrieve updated chat list
            const chatList = await chatGroupService.getMyChatList(userId, {
              page: 1,
              limit: 10,
            });
      ...
      socket.on(
        "seenAllMessages",
        async ({ chatGroupId }: { chatGroupId: string }) => {
          try {
            const chatGroup = await ChatGroup.findById(chatGroupId);
            if (!chatGroup) throw new AppError(404, "Chat group not found");
            if (!chatGroup.participants.includes(userId as unknown as ObjectId))
            ...
            chatNamespace
              .to(chatGroupId)
              .emit("messagesSeen", { chatGroupId, userId });
          } catch (error) {
            socket.emit("error", {
              message: (error as AppError).message,
              status: (error as AppError).statusCode || 500,
      ...
      socket.on(
        ...
        async ({ chatGroupId }: { chatGroupId: string }) => {
          try {
            ...
            const messages = await messageService.getChatMessages(chatGroupId, {
              limit: 50,
              page: 1,
            });
