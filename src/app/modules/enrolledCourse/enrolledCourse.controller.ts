...
const bulkInsertIntoDB = catchAsync(async (req, res) => {
  ...
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
    message: 'Bulk course enroll successfully!',
    data: result,
  });
});

const enrollBundleCourses = catchAsync(async (req, res) => {
...
const bulkEnrollBundleCourses = catchAsync(async (req, res) => {
  const result = await EnrolledCourseService.bundleEnrollBundleCourses(req.body);
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
    message: 'Bulk bundle course enrolled successfully!',
    data: result,
  });
});

const getEnrolledStudent = catchAsync(async (req, res) => {
  const filters = pick(req.query, enrolledCourseFilterableFields);
  const options = pick(req.query, ['limit', 'page', 'sortBy', 'sortOrder']);

  const result = await EnrolledCourseService.getEnrolledStudent(
    filters,
    options
  );
  ...
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
    message: 'Get all enrolled students data fetched!',
    meta: result.meta,
    data: result.data,
  });
});
